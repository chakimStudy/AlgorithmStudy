...
public class SWEA_D4_1249_보급로 {
	...
	private static int dijkstra() {
		...
		while(true) {
			...
			if (visited[r][c]) continue; // 이미 꺼냈던 좌표이면 패스
			...
			for (int di = 0; di < 4; di++) {
				...
				if (nr >= 0 && nr < N && nc >= 0 && nc < N && !visited[nr][nc]
					...
					/*
					 * 여기서 pq에 이미 이 좌표가 있을 수 있는데 그냥 넣고 있음.
					 * 그래서 나중에 큐에서 뺼 때 같은 좌표가 또 나올 수 있음.
					 * 고로 이미 나왔던 좌표이면, 즉 visited가 true이면 패스하는게 좋음.
					 * 패스하지 않으면 불필요한 반복을 도는 것.(값은 갱신되진 않지만)
					 */
				}
			}
		}
